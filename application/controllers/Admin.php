...
class Admin extends CI_Controller
{
    ...
    //manage attendance view of teacher
    ...
    function attendance_selector()
    {
        ...
        redirect(site_url('admin/manage_attendance_view_/' .$data['class_id'] . '/' . $data['section_id'] . '/' . $data['timestamp']), 'refresh');
    }
        ...
        function manage_attendance_view_teacher($class_id = '' , $section_id = '' , $timestamp = '')
        {
            if($this->session->userdata('admin_login')!=1)
                redirect(site_url('login'), 'refresh');
            ...
            $page_data['section_id'] = $section_id;
            $page_data['page_title'] = get_phrase('manage_attendance_of_teacher') . ' ' . $class_name . ' : ' . get_phrase('section') . ' ' . $section_name;
            $this->load->view('backend/index', $page_data);
        }
        ...
        function attendance_selector_teacher()
        {
            ...
            $data['timestamp']  = strtotime($this->input->post('timestamp'));
            $data['section_id'] = $this->input->post('section_id');
            $query = $this->db->get_where('attendance_teacher' ,array(
                'class_id'=>$data['class_id'],
                    'section_id'=>$data['section_id'],
                        'year'=>$data['year'],
                            'timestamp'=>$data['timestamp']
            ...
            if($query->num_rows() < 1) {
                ...
                foreach($teachers as $row) {
                    ...
                    $this->db->insert('attendance_teacher' , $attn_data);
                }
    
            }
            redirect(site_url('admin/manage_attendance_view_teacher/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['timestamp']),'refresh');
        }
    
        function attendance_update_teacher($class_id = '' , $section_id = '' , $timestamp = '')
        {
            $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
            //$active_sms_service = $this->db->get_where('settings' , array('type' => 'active_sms_service'))->row()->description;
            $attendance_of_teachers = $this->db->get_where('attendance_teacher' , array(
            ...
            redirect(site_url('admin/manage_attendance_view_teacher/'.$class_id.'/'.$section_id.'/'.$timestamp) , 'refresh');
        }
    ...
    function generate_bulk_student_csv($class_id = '', $section_id = '')
    {
        ...
        $line   = array('StudentName', 'Id', 'Email', 'Password', 'Phone', 'Address', 'Gender');
        fputcsv($file, $line, ',');
    ...
    function generate_bulk_teacher_csv()
    {
        ...
        $line   = array('Name', 'birthday', 'sex', 'religion', 'blood_group', 'Address', 'Phone', 'email', 'password', 'designation', 'social_links', 'about', 'show_on_website');
        fputcsv($file, $line, ',');
    ...
    function bulk_teacher_add_using_csv($param1 = '')
    {
        ...
        if ($param1 == 'import') {
            ...
            foreach ($csv as $row) {
                ...
                $data['birthday']  = $row[1];
                $data['sex']     =   ($row[2]);
                $data['religion']  = sha1($row[3]);
                $data['blood_group']     = $row[4];
                $data['address']   = $row[5];
                $data['phone'] = $row[6];
                $data['email'] = ($row[7]);
                $data['password'] = $row[8];
                $data['designation'] = $row[9];
                $data['social_links'] = $row[10];
                $data['about'] = $row[11];
                $data['show_on_website'] = $row[12];
                //student id (code) validation
                /*$code_validation = code_validation_insert($data['student_code']);
                 if(!$code_validation){
                     $this->session->set_flashdata('error_message' , get_phrase('this_id_no_is_not_available'));
