...
class Login extends CI_Controller
{
    ...
    {
        $this->load->view('backend/Etutorlog.php');
    }
    ...
    function validate_login()
    {
        ...
        //login for etutor
        $query = $this->db->get_where('etutor', $newcred);
        echo $query->num_rows();
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('etut_login', '1');
            $this->session->set_userdata('etut_id', $row->etutor_id);
            $this->session->set_userdata('login_user_id', $row->etutor_id);
            $this->session->set_userdata('name', $row->name);
            $this->session->set_userdata('login_type', 'etutor');
            redirect(site_url('etutor/dashboard'), 'refresh');
        }
